...
```{r}
homerange %>%
  ggplot(aes(x=taxon, y=log10.mass, fill=taxon)) +geom_boxplot(alpha=0.2, color="gray") +geom_violin(alpha=0.2) +coord_flip()
```
...
**Drug D lowers heart rate, while drug C generally raises it.**
```{r}
...
penguins %>%
  ...
  ggplot(aes(species, sex, fill=body_mass_g)) +geom_tile() +scale_fill_gradient(low="darkgreen", high="darkseagreen1")
```
