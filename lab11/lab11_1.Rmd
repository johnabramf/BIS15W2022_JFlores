...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "darkgoldenrod1", bins=40)+
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="deeppink", alpha  =0.4, color = "black")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x=log10.mass)) +
  geom_histogram(aes(y = ..density..), fill = "darkgoldenrod1", alpha = 0.4, color = "black")+
...
```{r}
homerange %>%
  ggplot(aes(x=log10.hra)) +geom_histogram(fill="cadetblue1", alpha=0.3, color="black") +labs(title="Histogram of log10 homeranges", x="Homerange (log10)")
```

2. Now plot the same variable using `geom_density()`.
```{r}
homerange %>%
  ggplot(aes(x=log10.hra)) +geom_density(fill="darkseagreen1", alpha=0.3) +labs(title="Density curve of log10 homeranges", x="Homerange (log10)", y="Density")
```

3. Combine them both!
```{r}
homerange %>%
  ggplot(aes(x=log10.hra)) +geom_density(color="tomato", alpha=0.3) +geom_histogram(aes(y = ..density..), fill="darkseagreen1", color="black", bins=20, alpha=0.3)+labs(title="Histogram and density curve of log10 homeranges")
```
...
homerange %>%
  ...
  ggplot(aes(x=taxon, y=log10.mass))+geom_boxplot(fill="khaki")+labs(title="Range of masses (log10) in animals with small homeranges")
```
