...
```r
?mean()
speedlist <- c(venom_GT,chiron,veyron)
mean(speedlist)
```
...
**If we have NAs in our data, we have to deal with them and fix them. Without fixing those holes, we woudln't be able to run our functions and code properly.**
