...
ui <- dashboardPage(
  ...
  selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm"),
  selectInput("y", "Select Y Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_depth_mm"),
   sliderInput("pointsize", "Select the Point Size", min = 1, max = 5, value = 3, step = 0.25)
  ), # close the first box
...
ui <- dashboardPage(
  dashboardHeader(title="Relative Abundance across Taxon"),
  dashboardSidebar(disable=T),
  dashboardBody(
    box(width=3,
        selectInput("x", "Select RA Taxon", choices=c("RA_Apes", "RA_Birds", "RA_Elephant", "RA_Monkeys", "RA_Ungulate", "RA_Rodent"), selected="RA_Elephant"),
        sliderInput("y", "Select the Point Size", min=0.5, max=5, step=0.5, value=2)
    ),
    box(title="Plot of Relative Abundance by Taxon", width=6,
      plotOutput("plot", width="500px", height="400px")
    )
  )
)
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    gabon %>%
      ggplot(aes_string(x="Distance", y=input$x)) +geom_point(size=input$y)+geom_smooth(method=lm, se=T)+theme_linedraw()
  })
session$onSessionEnded(stopApp)
}
