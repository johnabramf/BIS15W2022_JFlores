...
```{r}
penguins %>%
  ggplot(aes(x=flipper_length_mm, y=body_mass_g)) +geom_point() +theme_linedraw()
```
...
server <- function(input, output, session) {
  output$plot <- renderPlot({ggplot(homerange, aes_string(x="locomotion", fill=input$x))+geom_bar(position="dodge", color="black", alpha=0.8) + theme_linedraw()})
